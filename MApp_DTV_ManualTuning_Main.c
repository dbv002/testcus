...
EN_RET MApp_DTV_ManualTuning_Main(void)
{
    ...
    switch(enDtvManualTuningState)
    {
		...
		#if(ENABLE_S2_WORD)
            if( msAPI_DFT_GetTSSetting(stGenSetting.stScanMenuSetting.u16RFChannelNumber, &stTempTP) == TRUE)
		#else
		...
		#if(ENABLE_S2_WORD)
            stGenSetting.stScanMenuSetting.u16PreRFChannelNumber=stGenSetting.stScanMenuSetting.u16RFChannelNumber;
		#else
            ...
            MApp_ZUI_ProcessKey(u8KeyCode);
            {
                ...
                if(IsDTVInUse())
                {
                    ...
                    else
                    {
                        ...
                        {
						#if ENABLE_DVB_T2
                            if (DVB_T_DEMOD == MApp_DTV_Scan_T2_ScanCheckSignal() || MApp_DTV_Scan_T2_ForceMonitorT2Signal())
                            {
						...
						#endif
                            {
								if(msAPI_CM_GetCountry() == E_NORWAY)
								{
									...
									if((msAPI_DFT_VHF_CrossBWFreq_GetFinishScanStatus() == FALSE)
										&& (stGenSetting.stScanMenuSetting.u16RFChannelNumber >= 5 && stGenSetting.stScanMenuSetting.u16RFChannelNumber <= 120))
								#else
									if((msAPI_DFT_VHF_CrossBWFreq_GetFinishScanStatus() == FALSE)
